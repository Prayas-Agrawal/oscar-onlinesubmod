...
args = dict(
    ...
    batched=False,
    features_mode = "grads",
    # init_func=dict(mode = "div", frac=0.5),
    init_func=None,
    sampling_mode=None,
    submod_budget=0.1,
    batch_size=1024,
    ...
    val_grads_mode = "random", # doesnt matter here
    grads_mode = "v1",
    hessian_late_mean = False, # only matters when hessian_approx is true
...
class TrainClassifier:
    ...
    def get_funcs(self, div_funcs, rep_funcs, args, epoch, curr_step, num_batches):
        mode = args["init_func"]["mode"]
        frac = args["init_func"]["frac"]
        steps = int(num_batches*frac)
        if(curr_step < steps):
            ...
            for batch_idx, (images, targets) in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}")):
                ...
                if(args["warm_start"] <= epoch+1 and self.do_dss(args, step, epoch, allow_zero=True, total_steps=len(dataloader))):
                    ...
                    image_grads, val_grads = submod.calc_grads_features_samplewise(model, criterion, 
                                                                                images, targets, val_samples, args["batch_size"], args)
                    ...
                    if(args["features_mode"] == "grads"):
                        ...
                        image_features = image_features[:unknown] + 1e-6
                    ...
                    if(args["calc_sijs"]):
                        query_sijs = submod.get_sims_l2(image_features, val_features)
                        data_sijs = submod.get_sims_l2(image_features, image_features)
                        query_query_sijs = submod.get_sims_l2(val_features, val_features)
                        query_sijs, data_sijs, query_query_sijs = query_sijs.cpu().numpy(), data_sijs.cpu().numpy(), query_query_sijs.cpu().numpy()
                        # print("shapes, ", query_sijs.shape, data_sijs.shape, query_query_sijs.shape)
                    num_queries = val_features.shape[0]  
                    _metric = args["similarity_metric"]
                    
                    if(args["init_func"] is not None):
                        diversity = [
                            DisparitySumFunction(n = unknown, data = image_features, mode="sparse", metric=_metric,num_neighbors=5),
                            LogDeterminantFunction(unknown, mode="sparse", lambdaVal = 0.1,  data = image_features, metric=_metric, num_neighbors=5),
                        ]
                        representation = [
                            DisparityMinFunction(n = unknown, data = image_features, mode="sparse", metric=_metric,num_neighbors=5),
                            GraphCutFunction(n = unknown, data = image_features, mode="sparse", metric="cosine",num_neighbors=10, lambdaVal = 0.1),
                            FacilityLocationFunction(unknown, mode="sparse", data = image_features, metric=_metric, num_neighbors=5),
                        ]
                        funcs = self.get_funcs(diversity, representation, args, epoch, curr_step=batch_idx, num_batches=len(trainloader.dataset)//batch_size)
                    else: funcs = [
                        # submodlib.LogDeterminantMutualInformationFunction(unknown, lambdaVal = 0.1, metric=_metric,num_queries=num_queries,data_sijs=data_sijs, query_sijs=query_sijs, query_query_sijs=query_query_sijs),
                        submodlib.FacilityLocationMutualInformationFunction(unknown, num_queries=num_queries,  metric=_metric, data_sijs=data_sijs, query_sijs=query_sijs,),
                        submodlib.ConcaveOverModularFunction(unknown, num_queries=num_queries,  metric=_metric,query_sijs=query_sijs),
                        submodlib.GraphCutMutualInformationFunction(n = unknown,metric=_metric, num_queries=num_queries,query_sijs=query_sijs),
                        DisparityMinFunction(n = unknown, mode="dense", num_neighbors=4, sijs=data_sijs),
                        DisparitySumFunction(n = unknown, mode="dense", num_neighbors=4, sijs=data_sijs),
                    ]
                    ...
                    mode, greedyList, best_arm = submod.eps_greedy_composition_batched(model, val_samples, 
                                                                ...
                                                                val_grads=None if recalc_val else val_grads, logs=logs)
                    ...
                    opt_indices = submod.importance_sampling_batched(submod_indices, 
                                                                    prev_opt_indices, best_arm, args=args, 
                                                                    image_grads=image_grads, step_normed=step)
